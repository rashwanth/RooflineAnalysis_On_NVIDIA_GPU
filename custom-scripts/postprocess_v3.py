...
for idx, (tag, dfm) in enumerate(dfs.items()):
    ...
    for mem_type in memory_types:
        ...
        for i in range(len(LABELS)):
            ax.scatter(AI[i], FLOPS[i], color=color, marker=markers[mem_type], s=40, alpha=0.8,
                       label=tag if first_point else None)
            first_point = False
            all_AI.append(AI[i])
            all_GFLOPS.append(FLOPS[i])

# Define theoretical peak for DP (for RTX 4060)
...
xmin = min(all_AI) * 0.5
xmax = max(all_AI) * 2
ymin = max(min(all_GFLOPS) - 10, 1)
ymax = max(max(all_GFLOPS) + 20, peak_dp_gflops * 1.2)
...
ax.plot(x, np.minimum(bw_l1 * x, peak_dp_gflops), label='L1 : {:.1f} GB/s'.format(bw_l1), color='black', linestyle='--')
ax.plot(x, np.minimum(bw_l2 * x, peak_dp_gflops), label='L2 : {:.1f} GB/s'.format(bw_l2), color='black', linestyle='--')
ax.plot(x, np.minimum(bw_gddr6 * x, peak_dp_gflops), label='GDDR6 : {:.1f} GB/s'.format(bw_gddr6), color='black', linestyle='--')

# Plot only DP compute ceiling
